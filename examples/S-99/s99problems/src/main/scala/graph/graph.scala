package graph {
  ...
  abstract class GraphBase[T, U] {
    ...
    override def toString() = {
        def helper(set:Set[T], edges:List[Edge]): String = edges match {
            case Nil => set.mkString(", ")
            case head::Nil =>
                f"${head.n1.value}-${head.n2.value}/${head.value}"
            case head::tail =>
                f"${head.n1.value}-${head.n2.value}/${head.value}, "+helper(set-head.n1.value-head.n2.value, tail)
        }
        val set = Set()++nodes.keys
        "["+helper(set, edges)+"]"
    }
  }
