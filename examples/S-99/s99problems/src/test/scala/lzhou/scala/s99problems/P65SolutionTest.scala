...
class P65SolutionTest extends FunSuite {
    
    test("P65 Solution layoutBinaryTree2") {
        val tests = List(
                        ...
                        (Node('1', Node('a', Node('b', End, Node('c')), Node('d', Node('e'), End)), End), PositionedNode('1', PositionedNode('a', PositionedNode('b', End, PositionedNode('c', 2, 4), 1, 3), PositionedNode('d', PositionedNode('e', 4, 4), End, 5, 3), 3, 2), End, 7, 1))
                        )
        tests.foreach(test=>
            P65Solution.get.layoutBinaryTree2(test._1) must equal (test._2)
        )
    }
}
